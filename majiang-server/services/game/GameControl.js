...
const GameControl = {
	...
	startGame: function (message, ws) {
		...
		gameEvents.emit('gameEvent', { type: 'startGame', roomId: message?.roomId, playerId: null, data: {}, roomInfo, gameInfo, timestamp: moment().valueOf() })
	},
	/**
	 * 断线重连
	 * @param message
	...
	playCard: function (message, ws) {
		...
		gameEvents.emit('gameEvent', { type: 'playCard', roomId: data?.roomId, playerId: data?.userId, data: { cardNum: data?.cardNum }, roomInfo, gameInfo, timestamp: moment().valueOf() })
		// 3. 检测其他玩家是否需要打出的牌
	...
	peng: function (message, ws){
		...
		gameEvents.emit('gameEvent', { type: 'peng', roomId: data?.roomId, playerId: data?.userId, data: { pengArr: data?.pengArr }, roomInfo, gameInfo, timestamp: moment().valueOf() })
	},
	/**
	 * 杠
	 * @param message
	...
	gang: function (message, ws) {
		...
		gameEvents.emit('gameEvent', { type: 'gang', roomId: data?.roomId, playerId: data?.userId, data: { gangArr: data?.gangArr }, roomInfo, gameInfo, timestamp: moment().valueOf() })
	},
	/**
	 * 胡牌
	 * @param message
	...
	win: function (message, ws){
		...
		const winRoomInfo = RoomService.getRoomInfo(data?.roomId);
		const winGameInfo = RoomService.getGameInfo(data?.roomId);
		gameEvents.emit('gameEvent', { type: 'win', roomId: data?.roomId, playerId: data?.userId, data: { cardNum: data?.cardNum, result }, roomInfo: winRoomInfo, gameInfo: winGameInfo, timestamp: moment().valueOf() })
	}
}
